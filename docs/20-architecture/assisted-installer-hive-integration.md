...
RHACM/MCEでAssisted Installerを使う場合は、Hubクラスタ上のCRD、Assisted Service、Discovery ISOで起動したHost、完成後のSpokeクラスタを分けて見ると理解しやすくなります。

```mermaid
flowchart LR
  subgraph Hub["Hub Cluster (RHACM / MCE)"]
  ...
  subgraph Hosts["Bare Metal Hosts"]
    ISO["Discovery ISO"]
    HA["Host Agent"]
  end

  subgraph Spoke["Installed Spoke Cluster"]
    API["OpenShift API"]
    Nodes["Control Plane / Worker Nodes"]
  end
